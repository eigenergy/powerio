...
| `selectors.sub` | 514 | Every selector spelling: `BUS` lines, `BUSES a b`, `AREAS a b`, `ZONE`, `OWNER`, `KVRANGE`, a bare name, the `SYSTEM` synonym, two `JOIN` groups, a one line `SUBSYSTEM ... END`, a TARA `SCALE` line, tab and space indentation, a blank line, and no file `END`. |
