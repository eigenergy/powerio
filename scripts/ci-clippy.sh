...
Targets:
  ...
  capi-release    powerio-capi with arrow,matrix,gridfm,dist,prob.
...
case "$target" in
  ...
  capi-arrow)
    ...
    ;;
  capi-release)
    run cargo clippy -p powerio-capi --all-targets --features arrow,matrix,gridfm,dist,prob -- -D warnings
    ;;
  capi-dist)
