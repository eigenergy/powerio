...
pub enum SourceFormat {
    ...
    Normalized,
    /// Read from a GridFM data kit Parquet dataset. The balanced network follows
    /// the dataset's native dense bus indices, nodal demand and shunt totals,
    /// fixed quadratic costs, and branch terminal flows.
    #[serde(rename = "gridfm")]
    Gridfm,
...
pub(crate) struct BalancedNetworkTables {
    ...
    /// Component identities generated by PowerIO rather than supplied by the
    /// source. The provenance keeps target diagnostics from treating an
    /// internal indexing requirement as source data.
    #[serde(default, skip_serializing_if = "generated_uids_is_empty")]
    pub generated_uids: std::sync::Arc<BTreeSet<String>>,
...
impl BalancedNetwork {
    ...
    pub fn detailed_connectivity_mut(
        ...
        if let Some(detailed) = tables.detailed_connectivity.as_mut()
            && !detailed.omitted_fields.is_empty()
        {
    ...
    fn clear_omitted_fields(&mut self, component_type: &str, fields: &[OmittedFieldName]) {
        ...
        let affected = |omitted: &OmittedField| {
            omitted.component.component_type() == component_type && fields.contains(&omitted.field)
        };
        // Copy a shared connectivity only when a marker actually goes.
        if detailed.omitted_fields.iter().any(affected) {
            std::sync::Arc::make_mut(detailed)
                .omitted_fields
                .retain(|omitted| !affected(omitted));
        }
    }
    ...
    fn clear_omitted_field(&mut self, component: &ComponentId, field: OmittedFieldName) {
        ...
        let affected =
            |omitted: &OmittedField| omitted.component == *component && omitted.field == field;
        if detailed.omitted_fields.iter().any(affected) {
            std::sync::Arc::make_mut(detailed)
                .omitted_fields
                .retain(|omitted| !affected(omitted));
...
impl BalancedNetwork {
    ...
    pub fn uid_is_generated(&self, uid: Option<&str>) -> bool {
        uid.is_some_and(|uid| self.tables.generated_uids.contains(uid))
    }

    pub(crate) fn generated_uids(&self) -> &std::sync::Arc<BTreeSet<String>> {
        &self.tables.generated_uids
    }
...
fn generated_uids_is_empty(uids: &std::sync::Arc<BTreeSet<String>>) -> bool {
    uids.is_empty()
}
...
fn assign_missing_ids<T>(
    ...
    for value in values {
        ...
        let stem = stem(value);
        let suffix = match next_suffix.get_mut(&stem) {
            Some(suffix) => suffix,
            None => next_suffix.entry(stem.clone()).or_insert(1),
        };
        loop {
...
pub const GEN_EXTRA_KEYS: [&str; 11] = [
...
impl BalancedNetwork {
    ...
    pub(crate) fn check_references(&self, format: &'static str) -> crate::Result<()> {
        ...
        if let Some(detailed) = self.detailed_connectivity().as_ref() {
            ...
            let terminal_equipment: std::collections::HashSet<&ComponentId> = detailed
                .terminals
                .iter()
                .map(|terminal| &terminal.equipment)
                .collect();
            for junction in &detailed.junctions {
                ...
                if !terminal_equipment.contains(&junction.component) {
                    return Err(Error::FormatRead {
                        format,
                        message: format!(
            ...
            for endpoint in [&switch.endpoint1, &switch.endpoint2] {
                match endpoint {
                    TopologyEndpoint::Bus(bus) => {
                        if !configured_bus_levels.contains_key(bus) {
                            return Err(error(format!(
                    ...
                    TopologyEndpoint::Node(node) => {
                        if !connectivity_node_levels.contains_key(node) {
                            return Err(error(format!(
...
mod tests {
    ...
    fn assigned_component_ids_follow_records_after_reordering() {
        ...
        assert!(network.uid_is_generated(network.loads()[0].uid.as_deref()));
        ...
        assert!(restored.uid_is_generated(restored.loads()[0].uid.as_deref()));
    }

    #[test]
    fn source_and_generated_component_identities_remain_distinct() {
        let mut explicit = Bus::new(BusId(10), BusType::Ref, 230.0);
        explicit.uid = Some("source-bus".to_owned());
        let mut network = BalancedNetwork::new("identity provenance", 100.0);
        network
            .buses_mut()
            .extend([explicit, Bus::new(BusId(20), BusType::Pq, 230.0)]);
        network.assign_missing_component_ids();

        assert!(!network.uid_is_generated(network.buses()[0].uid.as_deref()));
        assert!(network.uid_is_generated(network.buses()[1].uid.as_deref()));
    }
