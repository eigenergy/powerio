...
0.10's `state_inventory`, `select_state`, and `export_state` replace the JSON
...
## The explicit balanced transformation

0.10's `lower_module_to_balanced` accepts a multiconductor module and returns a
balanced module with the records carried over and the pass's findings and
assumptions appended. Its 1.0 name is `transform::to_balanced`. The pass
transforms a supported three phase two winding `wye_delta`/`delta_wye`
transformer and merges an unrated identity
...
part of the series admittance the selected formula models. Every direct matrix
and vector calculation, the low level Rust `DcNetworkData`, and the C ABI
`PioDcData` spans carry the same sign. The internal positive factor weight is
the separate `DcConvention::calc_solver_edge_weight`, whose name states its role.
...
`calc_solver_edge_weight`. The preferred public coefficient calculation is
`calc_branch_susceptance`; the released noun spellings remain compatibility
aliases until 1.0. The relation is pinned by
...
`build_dc_opf_preparation`'s 1.0 `branches.susceptance_magnitude` field (the
0.10 `branches.b` field) is the elementwise negation of the
low level `DcNetworkData::susceptance` values, with an identical phase shift
injection. `DcNetworkData` and `dc_network_data` remain 0.10 compatibility
machinery and leave at 1.0 without a renamed high level container.
...
- Python: 0.10's `powerio.parse(source, from_, include_root=...,
  value_type=...)` replaces `parse_file`, `parse_str`, `parse_bytes`, and
  `read_pypsa_csv_folder`. In 1.0, paths use `parse_file` and in-memory text
  uses `parse_text`; the broad `parse` entry leaves the high level binding.
  `include_root`, omitted by default, widens the
...
`parse_file(path)` is the ordinary 0.10 call after `using PowerIO` and returns
`PioModule{T}` for the detected kind; `parse_bytes` covers memory and stream
input. In 1.0, in-memory text uses `parse_text`, and `parse_bytes` leaves the
ordinary export set. The `value_type` keyword, the type marker parse forms, the
...
findings as native `Diagnostic` records from `case.diagnostics`.
