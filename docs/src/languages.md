...
| typed narrowing | `try_into_typed::<T>(module)` (consuming, recoverable) | `value_type=` assertion + `module.value` | the `PioModule{T}` type itself; `case.value` | `pio_module_balanced_network`, `pio_module_multiconductor_network` |
...
| same format write | `write_module_str(&module, fmt)` | `module.to_json()` / `to_format` | `write_str(m; format)` / `write_file` | `pio_module_write_str` / `_write_file` |
...
- Errors follow the language: `Result` in Rust, exceptions in Python and Julia, status plus `PioError` handles in C. All four carry the same coded records.
