...
the `powerio` facade's `package` module gets a real error type. `from_json` returned one opaque `serde_json::Error` for malformed JSON, an unreadable lineage, and a `model_kind` contradicting its payload; those are `Malformed`, `UnsupportedVersion` and `ModelKindMismatch`.
