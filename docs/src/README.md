...
Supported sources: MATPOWER, PSS/E RAW revisions 32 through 35, PSS/E RAWX 35, PowSybl XIIDM and JIIDM 1.0 through 1.17, CIM CGMES 2.4.15 and 3.0, PowerWorld AUX and PWB, PSLF EPC, UCTE-DEF, the IEEE Common Data Format, PowerModels JSON, Egret JSON, pandapower JSON, PyPSA CSV folders, Surge JSON, DOE GO Challenge 3 JSON, DeepMind OPFData JSON, GridFM Parquet datasets, OpenDSS, PowerModelsDistribution engineering JSON, and BMOPF JSON. PowerIO IR enters through `deserialize`, not `parse`. [Formats and Fidelity](format-fidelity.md) states each format's supported profile and write support.
