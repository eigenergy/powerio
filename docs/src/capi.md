...
| 6 | The 1.0 handle model: every opaque handle gains a `retain`/`release` pair, new entry points return structured `PioError` handles, and the stored module (`pio_module_*`) and DC branch data (`pio_dc_data_*`) surfaces are added. The v5 entry points are unchanged. [Guide](abi-v6.md). |
