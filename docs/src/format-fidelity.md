...
repository README lists every format with its token and its read and write
support. This page states the numeric conventions, the independent checks
each reader and writer passes, and, format by format, what a reader keeps and
a writer reports.
...
with the result. Codes name the format and the reason, such as
...
read PowSybl IIDM 1.0 through 1.17 in the XML (`xiidm`)
...
reads 2.4.15 on CIM16 and 3.0 on CIM100. CGMES 2.4.15 uses the
...
class, field, count, and sample identities.

Fresh output is a deterministic
...
CGMES input retains every source `Substation`, including distinct
...
reads RAW revisions 32 through 35 and RAWX revision 35. RAW and
...
reads revisions 2003.09.01 and 2007.05.01 and writes
...
`.aux` is read and written. `.pwb` binary cases are read
only, and a `.pwd` display file parses to a `GeoLayer`
...
`.epc` is read and written. The reader maps the static power flow core:
...
canonical output (for a case that did not originate as MATPOWER)
...
output writes HVDC as `dc_branch`, the element its reader already
...
`TimeSeries<BalancedNetwork>` through `powerio::parse`; the component
crate's `powerio_tx::parse` reads only the static profile and refuses it.
### pandapower JSON

writes the power flow core as split oriented
...
are canonicalized directory outputs rather than byte
...
is a grid exchange format “for Challenge 3 and
...
reads and writes the versioned `surge-json` network document.
...
reads one raw JSON document from a FullTop or N-1
...
(`ieee-cdf`, alias `cdf`) is read only. The
...
(the `gridfm` feature, following the
...
findings. [Matrices and graphs](matrices.md) describes the dataset the writer
produces. Both directions need the `gridfm` cargo feature, which the CLI and
the Python wheel carry.
...
and reported with `READ.XIIDM.VERSION_COMPATIBILITY`; a namespace naming a
