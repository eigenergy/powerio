...
field name `approximation` because v0.10 already emitted that schema; its value
is the stable `BranchSusceptanceFormula` token, such as `series_susceptance`.
...
emit(scuc, "pio-json", "scenario_002.pio.json") # any kind stores as .pio.json
```
