...
| LinDist3Flow path matrices | (n−1)×(n−1), incidence (n−1)×(n−1) | `build_lindist3flow`, `build_lindist3flow_spanning_tree` | strict connected radial tree, or explicit minimum spanning tree projection |
...
- **LinDist3Flow.** `build_lindist3flow` requires one connected radial tree with
  ...
  path impedance matrices. `build_lindist3flow_spanning_tree` is the explicit
  meshed case projection: it keeps the minimum branch impedance spanning tree
  before building the same matrices. On a radial network with no shunts, the
  grounded LACPF block inverse is `[[R, X], [X, -R]]`.
