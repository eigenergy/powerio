...
Read it as an argued menu for ABI 6. The premise that drove its scope — that v5 would be the last breaking change, so everything had to land at once — is false. Every symbol resolves by `dlsym` behind an equality gate on `pio_abi_version`, powerio owns its one binding consumer, and that binding's artifact repin is automated. A later bump is routine.

Anything here that a consumer integration argues for belongs in an issue against ABI 6, never in a patch to this file. Nothing below has been rechecked against the source since it was written.
