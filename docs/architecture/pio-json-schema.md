...
2. **The payload** — the `model` field's `balanced_network` /
   ...
   **experimental**: it grows and changes whenever the IR does. PR #143, for
   example, added `DistLoad::voltage_model`, which appeared in the payload with
   no envelope version change. Do not treat the payload as a stable schema until
