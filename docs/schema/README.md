...
| Generation | First release | Document identity | Schema snapshot | Read by current |
|---|---|---|---|---|
...
PowerIO 0.11.3 keeps IR generation 2 and every existing record layout.
The fixed-dispatch LinDist3Flow instance and solution use distinct structural
type names. A reader accepts the types it implements; an older reader rejects an
...
structural types. The 0.11.1 snapshot adds the OPF pair; the 0.11.3 snapshot
adds the fixed-dispatch pair and supporting limit-report definitions. The
release name in the snapshot path does not create a new IR generation. A later
release without catalog changes can reuse that snapshot. CI checks that all
existing definitions and document rules remain identical and that published
