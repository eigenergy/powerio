...
Rust library + CLI/TUI. Parses power network case files and emits sparse matrices and graph views. Feeds the Scalable Approximate Cholesky solver and (planned) the GridFM ML pipeline.
...
- LACPF block `[[G, -B], [-B, -G]]` (Talkington flat start, 2n×2n, indefinite).
...
gridforge                                                    # TUI
gridforge batch -i tests/data -o out --matrices bprime,bdoubleprime
...
│   ├── triplet.rs           # CooBuilder (HashMap, O(nnz) inserts)
│   ├── bprime.rs
│   ├── bdoubleprime.rs
...
- **Sign convention.** Positive Laplacian: off diag negative, diag positive, `diag = sum |off-diag|` for B'. Matches the Talkington LACPF formulation and the Cholesky solver's ingest convention.
...
- **TUI lives in the library.** `src/tui/`. Testable via `ratatui::backend::TestBackend`. Binary calls `gridforge::tui::run`.
