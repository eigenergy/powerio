...
mod tests {
    ...
    /// A real ARPA-E GO Competition Challenge 3 case (`14bus_20220707.json`
    /// from GOCompetition's C3DataUtilities validation repo, vendored the
    /// same way PowerIO.jl's own `test/test_goc3_static.jl` cross-checks it),
    /// exercised at real scale. Counts are pinned to this scenario, matching
    /// the Julia side's own pinned assertions.
    #[test]
    fn scopf_data_matches_real_14bus_case_scale() {
        let text = include_str!("../tests/data/goc3_14bus_20220707.json");
