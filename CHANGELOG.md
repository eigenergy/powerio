...
**The MCP server owns explicit multiconductor to balanced lowering.** A `lower` tool takes `mode="preflight"` or `mode="apply"` on a multiconductor `.pio.json` package. Python has exposed `Package.multiconductor_to_balanced_preflight` and `Package.lower_multiconductor_to_balanced` since 0.9.0; over MCP a client could transport a multiconductor package and had no way to ask for either, so a balanced solver integration had to call the Python API locally or refuse the package. Preflight reports readiness, assumptions, approximations and the blocking `LOWER.MULTI_TO_BALANCED.*` records and changes nothing. Apply runs the same preflight and, on a blocker, returns `applied: false` with those blockers and no `package_json` — a structured refusal rather than a message, and no half-lowered result to mistake for a model; otherwise it returns the derived balanced package with its validation, diagnostics, origin and lowering history, which every balanced verb then takes unchanged. The `base_mva` and the sequence transform are reported either way, and `options` accepts `base_mva` alone: one transform exists, and a policy option accepted and ignored is the failure this tool exists to avoid. Nothing else lowers implicitly — `parse`, `summary`, `matrix`, `convert`, `save` and package readback keep a multiconductor payload multiconductor — and native multiconductor analysis stays the preferred path for a consumer that supports it (#398).
