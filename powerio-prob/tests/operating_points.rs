//! Operating point behaviors: identity
...
#[test]
fn scalar_builder_returns_an_operating_point() {
    let net = case9();
    let point = BalancedOperatingPointBuilder::for_point(net.clone())
        .bus_voltage_magnitudes(vec![1.0; net.buses().len()])
        .branch_in_service(vec![true; net.branches().len()])
        .build_point()
        .expect("scalar point builds");

    assert_eq!(point.bus_voltage_magnitude(BusId(1)), Some(1.0));
    assert_eq!(point.network().buses().as_ptr(), net.buses().as_ptr());
    assert_eq!(
        point
            .values(BalancedOperatingPointQuantity::BusVoltageMagnitude)
            .unwrap()
            .count(),
        net.buses().len()
    );
    assert!(
        point
            .flags(BalancedOperatingPointFlag::BranchInService)
            .unwrap()
            .all(|(_, in_service)| in_service)
    );
}
...
fn dense_columns_read_back_in_identity_order() {
    ...
    let vm: Vec<f64> = (0..2 * n)
        ...
        .collect();
    let series = BalancedOperatingPointBuilder::new(net.clone(), labels(2))
...
fn sparse_and_dense_storage_read_identically() {
    ...
    let first_load = net.loads()[0]
        .uid
        .clone()
        .expect("parsed components have persistent identities");
    let mut dense_service = vec![true; 3 * branches];
    dense_service[branches] = false;
    let first_branch = net.branches()[0]
        .uid
        .clone()
        .unwrap_or_else(|| "branches:0".to_owned());
    let dense_series = BalancedOperatingPointBuilder::new(net.clone(), labels(3))
        .load_active_powers(dense)
        .branch_in_service(dense_service)
        .build()
        .unwrap();
    let sparse_series = BalancedOperatingPointBuilder::new(net.clone(), labels(3))
        .sparse_load_active_powers(
            base,
            vec![Vec::new(), vec![(first_load.clone(), 99.0)], Vec::new()],
        )
        .sparse_branch_in_service(
            vec![true; branches],
            vec![Vec::new(), vec![(first_branch, false)], Vec::new()],
        )
        .build()
        .unwrap();
    for point in 0..3 {
        let d = dense_series.get(point).unwrap();
        let s = sparse_series.get(point).unwrap();
        ...
        for (row, branch) in net.branches().iter().enumerate() {
            ...
            assert_eq!(
                d.branch_in_service(&identity),
                s.branch_in_service(&identity),
                "point {point} branch {identity}"
            );
        }
    }
}
...
fn a_retained_point_survives_its_dropped_series_without_copying_the_network() {
    ...
    let series = BalancedOperatingPointBuilder::new(net, labels(2))
        .bus_voltage_angles(vec![0.25; 2 * n])
        .build()
        .unwrap();
    let point = series.get(1).unwrap();
...
fn every_point_shares_one_network_identity() {
    ...
    let n = net.buses().len();
    let bus_ptr = net.buses().as_ptr();
    let series = BalancedOperatingPointBuilder::new(net, labels(4))
...
fn unknown_and_duplicate_identities_are_refused() {
    let net = case9();
    let loads = net.loads().len();
    let error = BalancedOperatingPointBuilder::new(net.clone(), labels(1))
    ...
    let error = BalancedOperatingPointBuilder::new(duplicated, labels(1))
...
fn shape_mismatches_are_refused_loudly() {
    let net = case9();
    let n = net.buses().len();
    let error = BalancedOperatingPointBuilder::new(net.clone(), labels(2))
    ...
    let error = BalancedOperatingPointBuilder::new(net, Vec::new())
        .build()
...
fn an_unstated_quantity_reads_as_none_not_zero() {
    let net = case9();
    let n = net.buses().len();
    let series = BalancedOperatingPointBuilder::new(net, labels(1))
...
fn repeated_keyed_access_does_not_allocate() {
    ...
    let key = point.network().generators()[0]
        .uid
        .clone()
        .unwrap_or_else(|| "generators:0".to_owned());
    let key = key.as_str();
...
fn multiconductor_points_address_terminals_and_share_the_network() {
    ...
    let lp: Vec<f64> = (0..2 * load_conductors)
        ...
        .collect();
    let series = MulticonductorOperatingPointBuilder::new(net.clone(), labels(2))
