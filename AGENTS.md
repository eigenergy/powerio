...
- **A leading cost coefficient can be a rounding artifact.** A model 2 row that states a linear curve often carries a quadratic term near `1e-17`. `GenCost::quadratic_with_constant_tol(tol)` takes the leading coefficients at or below `tol` off the row before it counts the row; `GenCost::LEADING_COEFF_TOL` is `1e-12`. `quadratic_with_constant` reads the row as it stands and the OPF builders call it.
