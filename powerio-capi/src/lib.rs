...
//!
//! Functions parse, query, transform, and emit power system data through opaque handles. Feature
...
//!
//! - `parse_file` is the ordinary source entry. `parse_str` and `parse_bytes`
//!   remain for in-memory acquisition and ABI compatibility.
//! - `emit` produces an external representation. `write` and `convert` remain
//!   ABI compatibility names. `to_` names an in-memory semantic transform;
//!   older `to_json` and `to_arrow` representation functions keep their ABI
//!   names.
...
fn rebuilt_module<T: Clone>(
    ...
    let mut out = powerio_core::PioModule::new(module.value().clone())
        .with_producer(module.producer().clone());
...
pub unsafe extern "C" fn pio_multiconductor_network_graph_json(
    ...
    unsafe { pio_multiconductor_network_to_graph_json(net, error) }
}
...
#[cfg(feature = "dist")]
#[unsafe(no_mangle)]
pub unsafe extern "C" fn pio_multiconductor_network_to_graph_json(
    net: *const PioMulticonductorNetwork,
    error: *mut *mut v6::PioError,
) -> *mut c_char {
    unsafe {
        finish_string_v6(error, || {
            let net = net
                .as_ref()
                .ok_or_else(|| null_handle("distribution network handle"))?;
            serde_json::to_string(&net.net().to_graph())
                .map_err(|e| coded(&codes::EMIT_CAPI_SERIALIZE_FAILED, e))
        })
    }
}
...
mod tests {
    ...
    fn c_header_abi_manifest_is_pinned() {
        ...
        let expected = [
            ...
            "char *pio_multiconductor_network_to_graph_json(const PioMulticonductorNetwork *net, PioError **error);",
            ...
            "int32_t pio_module_emit_file(const PioModule *module, const char *format, const char *path, PioDiagnostics **out_diagnostics, PioError **error);",
            ...
            "char *pio_module_to_balanced_report_json(const PioModule *module, double base_mva, PioError **error);",
    ...
    mod dist {
        ...
        fn graph_json_reports_bus_edge_projection() {
            ...
            unsafe {
                ...
                let graph = pio_multiconductor_network_to_graph_json(net, &raw mut error);
                assert!(!graph.is_null(), "graph json failed: {}", error_text(error));
                let compatibility = pio_multiconductor_network_graph_json(net, &raw mut error);
                assert!(!compatibility.is_null());
                assert_eq!(CStr::from_ptr(compatibility), CStr::from_ptr(graph));
                pio_string_release(compatibility);
